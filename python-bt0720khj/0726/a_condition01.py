### 조건문(Condition) ###
# 특정 조건을 충족할 경우 측정 코드 블록을 실행하는 프로그래밍 구문
#if문 :만약 ~라면
...
'''
if 조건:
    (조건이 true 인 경우)실행할 코드

1. 조건 참(true) 또는 거짓(False)의 값을 갖는 ㅠㅛ현식
2. 실행할 코드: if문 바로 아래에 들여쓰기를 하여 작성

3if문의 맨 끝에는 조건 뒤에 클론(;)을 반드시 기재
'''
#2 기본 if 문
#어떤 수가 18보다 큰지 확인하는 조건문 작성
...
#3 else문
#if문의 조건이 풍족되지 않을 때 (즉, if문의 조건이 False 일때) 실행되는 코드 블록을 지정
...
# 삼항연산자
# value_if_true(실행된 코드-참) if condition(조건) else value_if_false(실행될 코드-거짓)
