...
print(a % b) # 모듈로(어떤 한 숫자를 다른 숫자로 나눈 나머지를 구하는 연산)
...
# : and 연산자는 모든 조건이  True 일 때  True를 변환
# : 그렇지 않으면 False를 반환

print(a or b) # True
# : or 연산자는 적어도 하나의 조건이 True일때 True를 변환
...
#변수 'd' 에 7을 할당하고, 변수 'd' 의 값에 3 을 더한 후 다시 'd' 에 저장하고 출력하세요
d = 7
d += 3

print(d)
#두 숫자가 모두 10보다 크거나 같은지 확인하는 코드를 작성하세요. 변수 'e' 와 'f' 에는 각각 12와9를 할당합니다
e = 12
f = 9

print(e >= 10 and f >= 10)
#변수 'g'에 True 를 할당하고 'g'의 블리언 값을 반전시킨 후 출력하세요
g = True
print(not g)
